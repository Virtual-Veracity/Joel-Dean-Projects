...
public class GameMap {
    ...
    private void initRegisterSpawners() {
        ...
        game.register(() -> game.spawnSpider(), Game.AI_MOVEMENT, "zombieToastSpawner");
    }
    ...
    public boolean containsZombies() {
        return getEntities(ZombieToast.class).size() != 0;
    }

    public List<Entity> getCardinallyAdjacentEntities(List<Position> positions) {
        List<Entity> entities = new ArrayList<>();
        positions.stream().forEach(p -> entities.addAll(getEntities(p)));
        return entities;
    }
}
