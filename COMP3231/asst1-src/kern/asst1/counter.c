 ...
 * 
 * Multiple counters can be initialised concurrently. You can assume
...
struct sync_counter * counter_initialise(int val)
{
        ...
        sc_ptr->counterLock = lock_create("counterLock");
        if (sc_ptr->counterLock == NULL) {
                panic("Lock failed to initialise");
        }
        // ==================================
...
int counter_read_and_destroy(struct sync_counter *sc_ptr)
{
        ...
        lock_acquire(sc_ptr->counterLock);
...
void counter_increment(struct sync_counter *sc_ptr)
{
        ...
        lock_release(sc_ptr->counterLock);
}
